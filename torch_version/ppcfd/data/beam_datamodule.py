...
class BeamDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.design_params = np.load(input_param_path)
        out_npz = np.load(output_npz_path)
        self.output_pos = out_npz["a"]
        self.output_vals = out_npz["b"]
        n_total, n_pts, _ = self.output_pos.shape

        if "mask" in out_npz:
            self.mask = out_npz["mask"].astype(np.bool_).reshape((n_total, n_pts, 1))
        else:
        ...
        t2 = time.time()
        print(f"Time cost of Data Pre-processing: {(t2 - t1):.2f}s")
    ...
    def _normalize(self):
        for name, arr in [
            ...
            if name == "xyz":
                self.output_pos = torch.from_numpy(new_arr.astype("float32"))
            elif name == "stress":
                self.output_vals = torch.from_numpy(new_arr.astype("float32"))
            elif name == "param":
                self.design_params = torch.from_numpy(new_arr.astype("float32"))
    ...
    def inverse_transform(self, tensor):
        ...
        tensor_np = tensor.detach().cpu().numpy().reshape(-1, scaler.n_features_in_)
        denorm_np = scaler.inverse_transform(tensor_np)
        return torch.from_numpy(denorm_np).reshape(original_shape).to(device)
        ...
        for n_pts in pts_list:
            if n_pts == "original":
                ...
                for k in range(n_case):
                    name = f"Job_{k}"
                    max_points = max(max_points, xyzd[name].shape[0])
