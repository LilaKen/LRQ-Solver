...
@hydra.main(version_base=None, config_path="./configs", config_name="lrqsolver_beam.yaml")
def main(cfg: DictConfig):
    model = hydra.utils.instantiate(cfg.model)
    checkpoint = "./output/pqformer_beam/model_ep5000.pt"
    ...
    trunk = torch.as_tensor(trunk, dtype=torch.float32).unsqueeze(0)
    branch = torch.as_tensor(branch, dtype=torch.float32).unsqueeze(0)
    with torch.no_grad():
...
def visual_beam(p_label, p_predict, case_id):
    ...
    data = reader.GetOutput()
    point = vtk_to_numpy(data.GetPoints().GetData())
    p_err_np = np.abs(p_label - p_predict)
    p_label_vtk = numpy_to_vtk(p_label)
    p_predict_vtk = numpy_to_vtk(p_predict)
    p_err = numpy_to_vtk(p_err_np)
    p_label_vtk.SetName("Label")
    p_predict_vtk.SetName("Pred")
    p_err.SetName("Abs Err")
    data.GetPointData().AddArray(p_label_vtk)
    data.GetPointData().AddArray(p_predict_vtk)
