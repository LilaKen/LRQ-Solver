...
def parallelize(model, optimizer, config):
    ...
    if config.enable_cinn is True:
        ...
        model = paddle.jit.to_static(
            model, full_graph=True, backend='CINN', 
            input_spec = paddle.static.InputSpec(shape=[1, 32186, 7], dtype='float32'))
...
def setup_dataloaders(config, dataloader, dataloader_fn):
    """
    setup dataloader into parrallel

    Args:
        config (Config): information in yaml file
        dataloader (DataLoader): input original dataloader

    Returns:
        DataLoader: parrallelized dataloader
