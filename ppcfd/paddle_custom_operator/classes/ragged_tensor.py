...
class RaggedTensor:
    ...
    def __init__(self, r_tensor, internal=False):
        """Creates a `RaggedTensor` with specified paddle script object.
    ...
    def from_row_splits(cls, values, row_splits, validate=True, copy=True):
        ...
        if isinstance(values, list):
            values = paddle.to_tensor(data=values, dtype="float64")
        elif isinstance(values, np.ndarray):
            values = paddle.to_tensor(data=values)
        elif isinstance(values, paddle.Tensor) and copy:
            values = values.clone()
        if isinstance(row_splits, list):
            row_splits = paddle.to_tensor(data=row_splits, dtype="int64")
        elif isinstance(row_splits, np.ndarray):
            row_splits = paddle.to_tensor(data=row_splits)
        elif isinstance(row_splits, paddle.Tensor) and copy:
            row_splits = row_splits.clone()
        r_tensor = paddle.classes.my_classes.RaggedTensor().from_row_splits(
    ...
    def requires_grad(self):
        ...
        return not self.values.stop_gradient

    @requires_grad.setter
    def requires_grad(self, value):
        self.values.stop_gradient = not value
    ...
    def __add__(self, other):
        return RaggedTensor(self.r_tensor + self.__convert_to_tensor(other), True)
    ...
    def __convert_to_tensor(self, value):
        ...
        if isinstance(value, RaggedTensor):
            if (
                self.row_splits.shape != value.row_splits.shape
                or paddle.any(x=self.row_splits != value.row_splits).item()
            ):
                raise ValueError(
            ...
            return paddle.to_tensor(data=[value], dtype="float32").to(type(value))
        else:
