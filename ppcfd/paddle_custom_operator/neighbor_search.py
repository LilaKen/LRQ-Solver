...
import ppcfd.paddle_custom_operator.classes as classes
import ppcfd.paddle_custom_operator.ops as ops

# import src.paddle_custom_operator.return_types as return_types
...
class FixedRadiusSearch(paddle.nn.Layer):
    ...
    def forward(
        ...
        if points_row_splits is None:
            points_row_splits = paddle.to_tensor([0, points.shape[0]], dtype="int64")
        if queries_row_splits is None:
            queries_row_splits = paddle.to_tensor([0, queries.shape[0]], dtype="int64")
...
class RadiusSearch(paddle.nn.Linear):
    ...
    def forward(
        self, points, queries, radii, points_row_splits=None, queries_row_splits=None
    ):
        ...
        """
        if points_row_splits is None:
            points_row_splits = paddle.to_tensor([0, points.shape[0]], paddle.int_64)
        if queries_row_splits is None:
            queries_row_splits = paddle.to_tensor([0, queries.shape[0]], paddle.int_64)
...
class KNNSearch(paddle.nn.Linear):
    ...
    def forward(
        self, points, queries, k, points_row_splits=None, queries_row_splits=None
    ):
        ...
        """
        if points_row_splits is None:
            points_row_splits = paddle.to_tensor([0, points.shape[0]], paddle.int64)
        if queries_row_splits is None:
            queries_row_splits = paddle.to_tensor([0, queries.shape[0]], paddle.int64)
        result = ops.knn_search(
            ignore_query_point=self.ignore_query_point,
            return_distances=self.return_distances,
            metric=self.metric,
            points=points,
